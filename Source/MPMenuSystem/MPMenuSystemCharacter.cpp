...
void AMPMenuSystemCharacter::CreateGameSession() // Called when pressing the 1 key
{
...
void AMPMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccsessful)
{
	if (bWasSuccsessful)
	{
		if (GEngine)
		...
		if (World)
		{
			World->ServerTravel(FString("/Game/ThirdPerson/Maps/Map_Lobby?listen"));
		}
	}
	else
	{
...
void AMPMenuSystemCharacter::OnFindSessionsComplete(bool bWasSuccsessful)
{
	if (!OnlineSessionInterface.IsValid())
	{
		return;
		...
		for (auto Result : SessionSearch->SearchResults)
		{
			...
			if (GEngine)
			{
				GEngine->AddOnScreenDebugMessage(
					...
					15.f,
					FColor::Cyan,
					FString::Printf(TEXT("Session Id: %s, Owning user name: %s"), *id, *User)
				);
			}
...
void AMPMenuSystemCharacter::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type)
{
	if (!OnlineSessionInterface.IsValid())
	{
		return;
